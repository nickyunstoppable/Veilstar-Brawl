 ...
 */
export class VeilstarBrawlService {
  private baseClient: VeilstarBrawlClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new VeilstarBrawlClient({
  ...
  private createSigningClient(
    ...
    return new VeilstarBrawlClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new VeilstarBrawlClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
...
// Example: const veilstarBrawlService = new VeilstarBrawlService(contractId);
