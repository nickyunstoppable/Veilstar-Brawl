...
import { readEnvFile, getEnvValue, mergeEnvFile } from './utils/env';
...
async function testnetContractExists(contractId: string): Promise<boolean> {
  ...
  try {
    await $`stellar -q contract fetch --id ${contractId} --network ${NETWORK} --rpc-url ${RPC_URL} --network-passphrase ${NETWORK_PASSPHRASE} --out-file ${tmpPath}`;
    return true;
  } catch {
    return false;
  } finally {
...
if (shouldEnsureMock) {
  ...
  if (mockGameHubId) {
    ...
    try {
      const result =
        await $`stellar contract deploy --wasm ${mock.wasmPath} --source-account ${adminSecret} --network ${NETWORK} --rpc-url ${RPC_URL} --network-passphrase ${NETWORK_PASSPHRASE}`.text();
...
for (const contract of contracts) {
  ...
  try {
    ...
    const installResult =
      await $`stellar contract install --wasm ${contract.wasmPath} --source-account ${adminSecret} --network ${NETWORK} --rpc-url ${RPC_URL} --network-passphrase ${NETWORK_PASSPHRASE}`.text();
...
const envUpdates: Record<string, string> = {
  ...
  // Avoid persisting stale Groth16 vk-id overrides across redeploys.
  // Server will compute the vk_id from zk_circuits/.../verification_key.json when empty.
  ZK_GROTH16_VK_ID: "",
  ZK_FINALIZE_VK_ID: "",
...
for (const contract of allContracts) {
  envUpdates[`VITE_${contract.envKey}_CONTRACT_ID`] = deploymentContracts[contract.packageName] || "";
}

// Server-side strict ZK mode uses non-VITE env vars.
if (deploymentContracts["zk-groth16-verifier"]) {
  envUpdates["ZK_GROTH16_VERIFIER_CONTRACT_ID"] = deploymentContracts["zk-groth16-verifier"];
}

await mergeEnvFile('.env', envUpdates, {
  headerComment: [
...
console.log("✅ Updated managed keys in .env (preserved custom variables)");
